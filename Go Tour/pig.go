...
func roll(s score) (score, bool) {
	outtime := rand.Intn(6) + 1
	if outtime == 1 {
		return score{s.opponent, s.player, 0}, true
	}
	return score{s.player, s.opponent, outtime + s.thisTurn}, false
}
...
func roundRobin(strategies []strategy) ([]int, int) {
	...
	// 不能自己一个人玩
	gamesPerStrategy := gamePerSeries * (len(strategies) - 1)
